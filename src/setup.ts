...
export const createUmi = async (endpoint?: string, airdropAmount?: SolAmount) =>
  baseCreateUmi(endpoint ? endpoint : 'http://localhost:8899', undefined)
...
export const createTree = async (
  ...
  builder = builder.append(
    setDecompressibleState(context, {
      treeConfig:
        input.treeConfig ?? findTreeConfigPda(context, { merkleTree: merkleTree.publicKey }),
...
export const mint = async (
  ...
  const leafIndex = Number(
    input.leafIndex ?? (await fetchMerkleTree(context, merkleTree)).tree.activeIndex,
  );
